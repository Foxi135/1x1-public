...
return {
    load = function() 
        ...
        cam = {x=-100,y=-100,cx=0,cy=0,zoom=20,visibleChunks={}}
        do
        ...
        for i = 2, 50000 do
            table.insert(level.entitiesInChunks[0][0],true)

            table.insert(level.entities,{
                name = "test",
                x = math.random(0,level.mapSize-1),
                y = math.random(0,level.mapSize-1),
                cx = 0,
                cy = 0,
                id = i,
                drawID = spriteBatch:add(quadPallete[entityAtlas["test"].color],0,0,nil,.5,.5)
            })
        ...
        spriteBatch = love.graphics.newSpriteBatch(imageEntityPallete,50000,"stream")
    end,
    draw = function()
        love.graphics.push()
        ...
        for i,cx,cy in cam.eachVisibleChunk() do  -- i have to change this
        ...
        do
            ...
            --[[local spritew,spriteh = .5,.5
            local c = collision.isColliding(mx,my,mcx,mcy,spritew,spriteh)
            for k, v in pairs(c) do
                love.graphics.rectangle("fill",v.x+(v.cx+cam.cx)*level.mapSize,v.y+(v.cy+cam.cy)*level.mapSize,.5,.5)    
            end
            setColor(1,0,0,.5)

            love.graphics.rectangle(#c>0 and "fill" or "line",mx+(mcx+cam.cx)*level.mapSize,my+(mcy+cam.cy)*level.mapSize,spritew,spriteh)]]
    ...
    update = function(dt) --IMPLEMENT ENTITIES
        for i = 1, 3 do
