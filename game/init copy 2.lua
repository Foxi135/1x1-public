    ...
    load = function() 
        ...
        for i = 2, 50000 do
            ...
            table.insert(level.entities,{
                name = "test",
                x = math.random(0,(level.mapSize-1)*1000)/1000,
                y = math.random(0,(level.mapSize-1)*1000)/1000,
                cx = 0,
                cy = 0,
                id = i,
            })
        ...
        for i,cx,cy in cam.eachVisibleChunk() do
            if level.entitiesInChunks[cx] and level.entitiesInChunks[cx][cy] then
                ...
                if not chunk.spriteBatch then
                    chunk.spriteBatch = love.graphics.newSpriteBatch(imageEntityPallete,50000,"stream")
